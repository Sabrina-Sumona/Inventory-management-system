...
import { SupplierFinancialSettingForm } from "@/components/suppliers/SupplierFinancialSettingForm";
...
interface SupplierFinancialSettingPanelProps {
  supplier: Supplier;
  canCreate: boolean;
  canUpdate: boolean;
  canDelete: boolean;
...
export function SupplierFinancialSettingPanel({
  supplier,
  canCreate,
  canUpdate,
  canDelete,
  onClose,
}: SupplierFinancialSettingPanelProps) {
  const [
    financialSetting,
    setFinancialSetting,
  ] =
    useState<SupplierFinancialSetting | null>(
      null
    );
  ...
  useEffect(() => {
    ...
    supplierService
      ...
      .then((data) => {
        ...
        setFinancialSetting(
          data.supplier_financial_settings[0] ??
            null
        );
      })
      .catch((error: unknown) => {
        ...
        setErrorMessage(
          getApiMessage(
            error,
            "Unable to load supplier financial settings."
          )
        );
      })
      .finally(() => {
  ...
  async function handleSave(
    ...
    try {
      ...
      if (financialSetting) {
        savedSetting =
          await supplierService.updateSupplierFinancialSetting(
            financialSetting.id,
            {
              currency_code:
              ...
              allow_credit_purchase:
                payload.allow_credit_purchase,

              block_purchase_on_credit_limit:
                payload.block_purchase_on_credit_limit,

              default_purchase_discount_percent:
                payload.default_purchase_discount_percent,
              ...
              purchase_price_basis:
                payload.purchase_price_basis,

              default_purchase_order_term:
                payload.default_purchase_order_term,
        ...
        setSuccessMessage(
          "Supplier financial settings updated successfully."
        );
      } else {
        savedSetting =
          await supplierService.createSupplierFinancialSetting(
            payload
          );

        setSuccessMessage(
          "Supplier financial settings created successfully."
        );
      }
      ...
      setErrorMessage(
        getApiMessage(
          error,
          "Unable to save supplier financial settings."
        )
      );
    } finally {
      setIsSaving(false);
  ...
  async function handleDelete(): Promise<void> {
    ...
    const confirmed = window.confirm(
      `Delete financial settings for "${supplier.name}"?`
    );
    ...
    try {
      await supplierService.deleteSupplierFinancialSetting(
        financialSetting.id
      );
      ...
      setSuccessMessage(
        "Supplier financial settings deleted successfully."
      );
    } catch (error) {
      setErrorMessage(
        getApiMessage(
          error,
          "Unable to delete supplier financial settings."
        )
      );
    } finally {
      setIsDeleting(false);
  ...
  return (
    <>
            ...
            {isLoading ? (
              ...
              <div className="rounded-xl border border-dashed border-slate-300 bg-slate-50 px-6 py-16 text-center">
                ...
                <p className="mx-auto mt-2 max-w-xl text-sm text-slate-500">
                  This supplier currently uses
                  only the basic payment terms
                  stored in the supplier
                  profile.
                </p>
              ...
              <>
                <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                  ...
                  <div className="rounded-xl border border-slate-200 p-5">
                    ...
                    <p className="mt-2 text-lg font-bold text-slate-950">
                      {
                        financialSetting.default_purchase_discount_percent
                      }
                      %
                    </p>
                  </div>
                ...
                <section className="rounded-xl border border-slate-200">
                  ...
                  <dl className="grid gap-5 p-5 md:grid-cols-2">
                    ...
                    <div>
                      ...
                      <dd className="mt-1 text-sm font-semibold text-slate-800">
                        {formatLabel(
                          financialSetting.default_purchase_order_term
                        )}
                      </dd>
                    </div>
      ...
      {isFormOpen && (
        <SupplierFinancialSettingForm
          key={
            financialSetting?.id ??
            `new-financial-setting-${supplier.id}`
          }
          supplier={supplier}
          financialSetting={
            financialSetting
